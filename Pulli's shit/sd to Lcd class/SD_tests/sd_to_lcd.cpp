...
void sd2Lcd::Init(int r, int c){
  if(r <= 0 || c <= 0){
    PrintMsg(Error, "rows and columns should be >= 0");
    PrintMsg(Info, "setting rows and cols to default value: r = 2, c = 16");
    r = 2;
    c = 16;
  }
  rows = r;
...
int sd2Lcd::find_index(char* buff, const char separator){
  if(buff == NULL){
    PrintMsg(Error, "got NULL buff, use read_line_sd() first!");
    return -1;
  }
	int index;
	char* p;
